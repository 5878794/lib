...
DEVICE.mulit_touch_evnet = function () {
    function multiTouchEvent(opt) {
        _classCallCheck(this, multiTouchEvent);
    ...
    _createClass(multiTouchEvent, [{
        ...
        value: function _addEvent() {
            ...
            window.addEventListener(DEVICE.CANCEL_EV, function (e) {
                e = e.touches || e;
                _this._endEvent(e);
            }, false);
        }
        ...
        key: "_savePoint",
        value: function _savePoint(e) {
            var new_e = JSON.parse(JSON.stringify(e));
            this.points.push(new_e);
        }
    }, {
        key: "_delPoint",
        value: function _delPoint() {
            this.points = [];
            this.startPoints = null;
            this.touchBeforeMoveNumber = 0;
        }
    }, {
        key: "_handler2Touches",
        value: function _handler2Touches() {
            ...
            var abs = Math.abs,
                sqrt = Math.sqrt,
                s1_x = startPoints[0].screenX,
                //起始点1
            s1_y = startPoints[0].screenY,
                s2_x = startPoints[1].screenX,
                //起始点2
            s2_y = startPoints[1].screenY,
                e1_x = endPoints[0].screenX,
                //结束点1
            e1_y = endPoints[0].screenY,
                e2_x = endPoints[1].screenX,
                //结束点2
            e2_y = endPoints[1].screenY,
                c_x = s1_x + (s2_x - s1_x) / 2,
                //中心点1
            ...
            var scale = move >= 0 ? this.scale + move / 100 : this.scale + move / 100;
            scale = DEVICE.getBetweenNumber(scale, this.minScale, this.maxScale);
            this.scale = scale;

            var deg = deg1 > deg2 ? deg1 : deg2;
        ...
        key: "_clockwise",
        value: function _clockwise(p1, p2, p3) {
            var dx1 = p1.x - p2.x,
                dy1 = p1.y - p2.y,
                dx2 = p3.x - p2.x,
                dy2 = p3.y - p2.y,
